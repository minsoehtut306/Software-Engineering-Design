...
/**
 * The FeatureAccessServiceTest class tests the functionality of the
 * FeatureAccessService
 * in the context of user interactions, particularly focusing on how different
 * user types
 * access different features within the system. This class employs Mockito to
 * simulate
 * user interactions and uses assertions to ensure that the correct features are
 * displayed
 * for different types of users, and that appropriate messages are displayed
...
public class FeatureAccessServiceTest {
    ...
    void displayFeatureOptionsTestEncostUserAccess() {
        ...
        User mockUser = new User(UserType.encostVerified);

        if (mockUser.getUserType() == UserType.encostVerified) {
    ...
    void displayFeatureOptionsTestCommunityAccess() {
        ...
        User mockUser = new User(UserType.community);
        if (mockUser.getUserType() == UserType.community) {
            // Act
            // This method needs to read from System.in and write to System.out
            featureAccessService.displayFeatureOptions(mockUser);
        }
    ...
    void displayFeatureOptionsUnverifiedEncostAccess() {
        ...
        User mockUser = new User(UserType.encostUnverified);
        if (mockUser.getUserType() == UserType.encostUnverified) {
    ...
    void displayFeatureOptionsTestCommunityInvalidInput() {
        ...
        // Assert
        // Checks whether the corresponding message informs the community user of an
        // invalid
        // input
    ...
    void displayFeatureOptionsTestEncostVerifiedCustomDatasetInput() {
        // Arrange
        User mockUser = new User(UserType.encostVerified);
        if (mockUser.getUserType() == UserType.encostVerified) {
